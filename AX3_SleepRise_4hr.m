function [slprise,block_final]=AX3_SleepRise_4hr(wtv_instance,wtv)
% [slprise,block_final]=AX3_SleepRise_4hr(wtv_instance,wtv)
% 
% version 2
    ...
    if(chk_24hr_sleep>240) %only continue if monitor was off for at least 4 hrs
        ...
        for jx=1:size(tmpx,2);
            ...
            if(wtv_instance{tmpx(jx),2}==1 && wtv_instance{tmpx(jx),3}>=60)
                ...
                if(sum01<30 && sum12>=(tmp_tlen/4))
                    start_good = 1;
                end
                if(sum23<30 &&  sum12>=(tmp_tlen/4))
                    end_good = 1;
                end
